...
// Dependent upon the schema of the OpenRPC json files generated by "npm run build:openrpc" 
...
// Build a method map for a single SDK (given by an object read from OpenRPC json file)
...
function getMethod(methodName) {
  const supportedSdks = Object.keys(config.app.supportedSdks);
  for ( let ii = 0; ii < supportedSdks.length; ii += 1 ) {
    const sdkName = supportedSdks[ii];
    if ( methodMaps[sdkName] && methodName in methodMaps[sdkName] ) { return methodMaps[sdkName][methodName]; }
  }
  return undefined;
}
...
function getSchema(schemaName) {
  const supportedSdks = Object.keys(config.app.supportedSdks);
  for ( const ii = 0; ii < supportedSdks.length; ii += 1 ) {
    const sdkName = supportedSdks[ii];
...
// Load the OpenRPC json file for the given SDK, if that SDK is enabled
async function readSdkJsonFileIfEnabled(sdkName) {
  let openRpcFileName;
  let url;
  if ( isSdkEnabled(sdkName) ) {
    try {
      openRpcFileName = config.app.supportedSdks[sdkName];
      url = new URL(`./${openRpcFileName}`, import.meta.url);
...
// Load the OpenRPC json files for any/all SDKs that are enabled ('core' always; others optionally)
// NOTE: Assumes the build process has put the OpenRPC json files in the same directory
//       as the source code from the src/ directory
async function readAllEnabledSdkJsonFiles() {
  await Promise.all(Object.keys(config.app.supportedSdks).map(async (sdkName) => {
...
// The value for each key will be an object containing keys: openrpc, info, methods, and components (contents of the OpenRPC json file)
const rawMeta = {};
